...
def load_custom_css():
    st.markdown(
        ...
        html, body, div, h1, h2, h3, h4, h5, h6, p, a, span, label, button, textarea, select {
            font-family: 'Poppins', sans-serif !important;
            color: black !important;
        }

        /* Apply font weight specifically to headings */
        h1, h2, h3, h4, h5, h6 {
            font-weight: 600;
        }

        /* Style the content container with white background and enhanced shadow */
        .stApp .block-container {
            background-color: white !important;
            color: black !important;
            border-radius: 15px;
            padding: 2rem;
            box-shadow: 2px 8px px rgba(0, 0, 0, 0.15); /* Adjusted horizontal offset to 2px */
        }

        /* Styling for the results section with a thin black border and deeper shadow */
        .results-container {
            background-color: white !important;
            color: black !important;
            ...
            padding: 1.5rem;
            border-radius: 15px;
            box-shadow: 2px 8px 24px rgba(0, 0, 0, 0.1); /* Similar shadow for consistency */
            margin-top: 20px;
        }
...
def add_bg_image():
    st.markdown(
        ...
        .stApp .block-container {
            background-color: rgba(0, 0, 0, 0.65); /* Transparent dark background for readability */
            color: white; /* White text for contrast */
            border-radius: px; /* Rounded corners for the container */
            padding: 2rem; /* Padding inside the container */
            width: 80%; /* Set the width of the container */
            max-width: 900px; /* Set a maximum width */
            margin: 5rem auto; /* Center the container and add space on top */
            box-shadow: 0 4px 35px rgba(0,0,0,1); /* Add shadow for depth */
        }
        ...
        .markdown-text-container {
            ...
            background-color: rgba(255, 255, 255, 0.8); /* Add a light transparent background */
            position: relative;
...
if region == "NSW":
    ...
    enable_NDH = False  # Disable the NDH option for NSW
else:
...
if enable_NDH:
    NetDevelopableHectares = st.number_input(
        "Net Developable Hectares", value=20.0, step=1.0, format="%.2f"
    )
else:
    # When "Enable NDH" checkbox is not enabled, prompt for GrossHA and AssumedNDA
    GrossHA = st.number_input(
        "Gross Hectare", value=20.0, step=1.0, format="%.2f"
    )
    AssumedNDA = st.number_input(
        "Net Developable Hectares %", min_value=1, max_value=100, value=100
    )
